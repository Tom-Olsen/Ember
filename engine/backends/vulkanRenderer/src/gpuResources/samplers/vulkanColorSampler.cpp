...
namespace vulkanRendererBackend
{
	...
	ColorSampler::ColorSampler(Settings settings)
	{
		VkPhysicalDeviceProperties properties{};
		vkGetPhysicalDeviceProperties(Context::GetVkPhysicalDevice(), &properties);

		VkSamplerCreateInfo samplerInfo = { VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO };
		samplerInfo.magFilter = settings.filter;							// magnified texels filter
		samplerInfo.minFilter = settings.filter;							// minified texels filter
		...
		samplerInfo.mipmapMode = settings.mipmapMode;						// how to handle mipmapping
		...
		NAME_VK_OBJECT(m_sampler, settings.name);
	}
