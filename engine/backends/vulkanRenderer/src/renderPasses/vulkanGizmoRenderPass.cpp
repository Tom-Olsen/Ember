...
namespace vulkanRendererBackend
{
	...
	void GizmoRenderPass::CreateRenderPass()
	{
		...
		dependencies[0].dstStageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT;							            // color attachment output must wait until the layout transition into color attachment layout is complete.
	...
	void GizmoRenderPass::CreateFrameBuffers()
	{
		...
		for (size_t i = 0; i < imageCount; i++)
		{
			...
			attachments[1] = m_pRenderTextures[i]->GetVmaImage()->GetVkImageView(); // msaa on: resolve msaa images to final render texture (skipped implicitly with msaa off, as this attachment doesn't get bound).
