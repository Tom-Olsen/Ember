...
{
	// Public methods:
	void OutlineStage::Record(const FrameContext& frameContext)
	{
		PROFILE_FUNCTION();

		// Prepare command recording:
		CommandPool& commandPool = frameContext.resources.GetCommandPool(RenderStage::outline);
		...
		VKA(vkBeginCommandBuffer(commandBuffer, &beginInfo));
		{
			...
			VkViewport viewport = {};
			viewport.width = pOutlineRenderPass->GetRenderTexture(frameContext.frameIndex)->GetWidth();
			viewport.height = pOutlineRenderPass->GetRenderTexture(frameContext.frameIndex)->GetHeight();
			...
			if (!frameContext.frameRenderData.outlineDrawCalls.empty())
			{
				// Pipeline:
