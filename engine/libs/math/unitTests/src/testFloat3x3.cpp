...
TEST(Float3x3, Determinant)
{
	...
	EXPECT_TRUE(math::IsEpsilonEqual(det, 3.0f));
}
...
TEST(Float3x3, RotateFromTo)
{
	...
	Float3 v0 = Float3(1.0f, 0.0f, 0.0f);
	Float3 v1 = rotMatrix * v0;
	EXPECT_TRUE(v1.IsEpsilonEqual(to));
}
...
//	EXPECT_TRUE(Float3::right.IsEpsilonEqual(right));
//	EXPECT_TRUE(Float3::backward.IsEpsilonEqual(backward));
//}
...
TEST(Float3x3, OperatorMultiplication)
{
	...
	EXPECT_TRUE(mult.GetRow(0) == Float3(30, 24, 18));
	EXPECT_TRUE(mult.GetRow(1) == Float3(84, 69, 54));
	EXPECT_TRUE(mult.GetRow(2) == Float3(138, 114, 90));
}
