...
namespace emberEngine
{
	...
	void DrawMeshData::LateUpdate()
	{
		...
		for (uint32_t i = 0; i < m_pMesh->GetVertexCount(); i++)
		{
			...
			// Draw calls:
			m_shaderProperties[0] = Renderer::DrawMesh(*m_pSphereMesh, m_material, Float4x4::TRS(worldPosition, Float3x3::identity, Float3(0.1f)), receiveShadows, castShadows);
			m_shaderProperties[0].SetValue("SurfaceProperties", "diffuseColor", Float4(0.66f, 0.33f, 0.0f, 1.0f));

			m_shaderProperties[1] = Renderer::DrawMesh(*m_pArrowMesh, m_material, Float4x4::TRS(worldPosition, Float3x3::RotateFromTo(Float3::forward, worldNormal), Float3(0.1f)), receiveShadows, castShadows);
			m_shaderProperties[1].SetValue("SurfaceProperties", "diffuseColor", Float4(0.0f, 0.0f, 1.0f, 1.0f));

			m_shaderProperties[2] = Renderer::DrawMesh(*m_pArrowMesh, m_material, Float4x4::TRS(worldPosition, Float3x3::RotateFromTo(Float3::forward, worldTangent), Float3(0.1f)), receiveShadows, castShadows);
