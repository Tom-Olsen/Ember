...
StructuredBuffer<float2> sourcePositionBuffer : register(t105, CALL_SET);
StructuredBuffer<float2> sourceVelocityBuffer : register(t106, CALL_SET);
...
void main(uint3 threadID : SV_DispatchThreadID)
{
    ...
    if (index < pc.threadCount.x)
    {
        ...
        float2 position = sourcePositionBuffer[index] + (a1 * kp1Buffer[index] + a2 * kp2) * dt;
        float2 velocity = sourceVelocityBuffer[index] + (a1 * kv1Buffer[index] + a2 * kv2) * dt;
