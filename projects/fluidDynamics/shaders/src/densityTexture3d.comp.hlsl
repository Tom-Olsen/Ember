...
void main(uint3 threadID : SV_DispatchThreadID)
{
    if (threadID.x < pc.threadCount.x && threadID.y < pc.threadCount.y && threadID.z < pc.threadCount.z)
    {
        ...
        float3 localPosition = boundsMin + (float3(threadID) + 0.5f) * voxelSize;
        float3 center = 0.5f * (boundsMin + boundsMax);
        float3 voxelCenterWorldPosition = center + mul(rotation, float4(localPosition - center, 0.0f)).xyz;

        float density = 0.0f;
        int3 voxelCell = HashGrid3d_Cell(voxelCenterWorldPosition, effectRadius);
        for (uint i = 0; i < 27; i++)
        {
            ...
            {
                float3 offset = voxelCenterWorldPosition - positionBuffer[otherIndex];
