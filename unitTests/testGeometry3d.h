...
{
	Float3 direction = mathf::Random::UniformDirection();
...
TEST(geometry3d, PointToPlaneDistance)
{
	...
	for (int i = 0; i < 100; i++)
		for (int j = 0; j < 100; j++)
		{
			...
			float distance = mathf::Random::Uniform(-50.0f, 50.0f);
			Float3 point = planeSupport + dir0 * i + dir1 * j;
...
TEST(geometry3d, LinePlaneIntersection)
{
	...
	for (int i = 0; i < 100; i++)
		for (int j = 0; j < 100; j++)
		{
			...
			lineDirection += dir0 * mathf::Random::Uniform(-10.0f, 10.0f);
			lineDirection += dir1 * mathf::Random::Uniform(-10.0f, 10.0f);
			lineDirection.Normalize();
			
			// Line support at random distance:
			float distance = mathf::Random::Uniform(-20.0f, 20.0f);
