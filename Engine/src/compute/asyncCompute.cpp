...
namespace emberEngine
{
	...
	namespace compute
	{
		...
		void Async::DispatchComputeSession(uint32_t sessionID)
		{
		...
		void Async::RecordBarrier(uint32_t sessionID, AccessMask::ComputeShader srcAccessMask, AccessMask::ComputeShader dstAccessMask)
		{
			if (s_computeSessions[sessionID].state != ComputeSession::State::recording)
			{
				LOG_ERROR("compute::Async::RecordComputeShader(...) failed. sessionID: {} is not in recording state.", sessionID);
			...
			ComputeCall computeCall = { 0, Uint3::zero, nullptr, nullptr, AccessMask::Convert(srcAccessMask), AccessMask::Convert(dstAccessMask) };
			s_computeSessions[sessionID].RecordComputeCall(computeCall);
		}
