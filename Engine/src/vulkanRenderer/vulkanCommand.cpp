...
namespace emberEngine
{
	...
	VulkanCommand::VulkanCommand(VulkanQueue queue)
	{
		...
		VKA(vkCreateCommandPool(VulkanContext::GetVkDevice(), &createInfo, nullptr, &m_pool));
		...
		VKA(vkAllocateCommandBuffers(VulkanContext::GetVkDevice(), &allocateInfo, &m_buffer));
	}
	VulkanCommand::~VulkanCommand()
	{
		vkDestroyCommandPool(VulkanContext::GetVkDevice(), m_pool, nullptr);
	}
	...
	// Static methods:
	VulkanCommand VulkanCommand::BeginSingleTimeCommand(const VulkanQueue& queue)
	{
		VulkanCommand command(queue);
		VkCommandBufferBeginInfo beginInfo = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
		beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
		VKA(vkBeginCommandBuffer(command.GetVkCommandBuffer(), &beginInfo));
		return command;
	}
	void VulkanCommand::EndSingleTimeCommand(const VulkanCommand& command, const VulkanQueue& queue, bool waitQueueIdle)
	{
