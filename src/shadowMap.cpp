...
// static members:
uint32_t ShadowMap::shadowMapWidth = 2048;
uint32_t ShadowMap::shadowMapHeight = 2048;
...
ShadowMap::ShadowMap(VulkanContext* context)
{
	...
	uniformBuffer = std::make_unique<VulkanUniformBuffer>(context, sizeof(VulkanUniformBuffer));
...
void ShadowMap::CreateRenderpass()
{
	...
	attachment.format = shadowFormat;
...
void ShadowMap::UpdateDescriptorSets()
{
	for (uint32_t frameIndex = 0; frameIndex < context->framesInFlight; frameIndex++)
	{
		...
		descriptorWrite.dstBinding = 0;		// hard coded as b0 in shadow.hlsl shader
...
std::vector<char> ShadowMap::ReadShaderCode(const std::filesystem::path& spvFile)
{
	// Open shader file:
