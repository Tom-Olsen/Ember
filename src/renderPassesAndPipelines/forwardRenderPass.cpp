...
namespace emberEngine
{
	...
	void ForwardRenderPass::CreateMsaaImage()
	{
		...
		m_msaaImage = std::make_unique<VmaImage>(m_pContext, imageInfo, allocationInfo, subresourceRange, viewType, queue);
	}
	void ForwardRenderPass::CreateDepthImage()
	{
		...
		VkImageViewType viewType = VK_IMAGE_VIEW_TYPE_2D;
		VulkanQueue queue = m_pContext->pLogicalDevice->GetGraphicsQueue();
		m_depthImage = std::make_unique<VmaImage>(m_pContext, imageInfo, allocationInfo, subresourceRange, viewType, queue);
